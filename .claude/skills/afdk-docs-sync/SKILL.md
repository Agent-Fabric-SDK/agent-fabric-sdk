...
`website/` (Nextra) describes how the SDK behaves from a developer's
...
| `python/src/agent_fabric/llm/*` (`catalog.py`, `client.py`) | `website/pages/feature-overview.mdx`, `website/pages/concepts/attribution.mdx` | Model catalog shape, capability flags, and the live-verified attribution headers (Pillar 1). |
...
| `python/src/agent_fabric/provisioning/publish.py` | `website/pages/provisioning/index.mdx`, `website/pages/publishing.mdx` | Pillar 3 overview and its publish-symmetry-with-Governance framing. |
...
The mapping above targets the consumer-facing `website/`. Two in-repo
...
README(s) in the same PR. When in doubt, the website page wins and the example
...
- The PR already touches `website/` or `docs/`.
...
1. Open the mapped `website/pages/**.mdx` file and the source file(s) that
   changed.
...
4. Run the repo's normal gates before pushing — `pytest -q`, `mypy`,
   ...
   these; a docs-only change to `website/` can be checked with
   `npm run build` from `website/` if you touched MDX syntax.
...
| "docs/verified-apis.md is engineering-internal, not website" | It feeds two live website pages (`concepts/verification.mdx`, `reference/unsupported-boundary.mdx`) plus the README status banner — a status flip there is a website change by another name. |
...
1. Add a row here — file pattern on the left, exact `website/pages/**.mdx`
   path on the right, one-line "why."
2. Verify the target `.mdx` file actually exists under `website/pages/`
