---
name: ddk-implementing-features
description: Use when actually writing the code for a feature in donkey-development-kit — after the issue + branch exist and before the PR. Triggers on "implement feature X", "build the budget/OTel/simulate feature", "wire this onto the transport hook", "attach to the skeleton", "add an adapter for <framework>", "add a new rejection type", "extend classify()", "add a config setting". Owns the IMPLEMENT stage between [[ddk-git-workflow]] step 3 (brainstorm + boundaries) and step 4 (commit/push/PR). Routes to the right pattern; defers code-quality rules to [[ddk-coding-conventions]] and verification to [[ddk-verification-discipline]].
---

# DDK Implementing Features

## Overview

This skill covers the **IMPLEMENT** stage — you are on a correctly-named issue
branch ([[ddk-git-workflow]]) and about to write code. It exists because the
...
[[ddk-coding-conventions]] and are assumed here. It does not restate the
verification contract — that is [[ddk-verification-discipline]]. This skill is
...
`DonkeyAsyncClient` (mirrored on the sync `DonkeyClient`). It exposes four
...
| `_swap_transport` | fixture seam | `simulate()` (#190) / `donkey mock` (#187) (`BG §1.4`/`BG §1.5`) |
...
The live budget surface is `donkey.budget.remaining` / `.reset_at` /
...
`simulate()`. Pick the hook from the table, override it on `DonkeyAsyncClient`
**and** its sync twin `DonkeyClient` (cross-surface lockstep — the two must not
diverge), obey the three contracts, and leave the default a no-op when the
collaborator isn't attached. Tests: [[ddk-testing]] Surface 1
(`tests/unit/`). If the feature reads a new response header whose name isn't
confirmed, that is a [[ddk-verification-discipline]] question — use
...
§2): the `donkey.<framework>` factory, a `connection_kwargs()` accessor, and a
module-level factory. Do this in order:

1. **Write `python/src/donkey_kit/integrations/<framework>.py`.** Import the
...
2. **Register it in `ADAPTERS`** (`integrations/__init__.py`): key, module path,
   ...
   `donkey.<key>` resolves to.
3. **Verify the curated `ImportError`.** `Donkey.__getattr__` must raise an
   `ImportError` carrying the exact `pip install 'donkey-kit[<extra>]'`
...
5. **Wire the conformance suite** (`python/tests/conformance/suite.py`): the one
   ...
   **asserted exemption in `KNOWN_LIMITATIONS`** — *never* a silent skip
   (§8.1). See [[ddk-testing]] for which surface each scenario belongs to.
6. **Add the `scripts/verify_frameworks.py` entry** so the offline
   ...
   [[ddk-verification-discipline]] step, not a guess.
7. **Add `python/examples/<framework>/`** (the eight existing dirs are the
   ...
   [[ddk-docs-authoring]]; whether it's required *in this PR* is
   [[ddk-docs-sync]].
...
- [[ddk-pr-review]] §3 (Error-taxonomy correctness) — the invariants a
  reviewer checks: never-retry-a-refusal, required `remediation`, discriminate
  on `type`+headers.
- [[ddk-testing]] Surface 1 — the mandatory assertions for any
  `core/errors.classify()` change, and the fixture layout under
  `tests/fixtures/`.
- [[ddk-verification-discipline]] — a rejection shape not yet captured live
...
1. Add the field to `DonkeyConfig`.
2. Resolve it in `from_env()` along the fixed precedence — constructor kwargs →
   env var → `.donkey-kit.toml` → default (§2.1). Missing required fields are
...
[[ddk-verification-discipline]] (`Unverified(...)` or `_verify.blocked(...)`).
...
- **Cross-surface lockstep** — a capability on more than one surface (Python /
  ...
  [[ddk-docs-sync]] to decide what must move *in this PR*.
...
- When you finish the code, hand back to [[ddk-git-workflow]] step 4
  (commit/push) and then [[ddk-pr-workflow]] for the pre-PR gate.
...
| "I'll use `budget.wait_for_reset()`." | It's planned (#186), not shipped. Use `donkey.budget.reset_at`. |
...
| "I don't know the exact kwarg, I'll use a sensible default." | That's what §0.3 forbids. `Unverified(...)` / `_verify.blocked(...)` and route through [[ddk-verification-discipline]]. |

## Related skills

- [[ddk-git-workflow]] — the branch you must already be on; steps 3→4 bracket
  this stage.
- [[ddk-coding-conventions]] — the code-quality rules this skill assumes.
- [[ddk-verification-discipline]] — any endpoint/header/class-name/kwarg claim.
- [[ddk-testing]] — which pytest surface proves the change.
- [[ddk-docs-sync]] / [[ddk-docs-authoring]] — whether/how `website/` moves.
- [[ddk-pr-workflow]] — the pre-PR gate once the code is written.
