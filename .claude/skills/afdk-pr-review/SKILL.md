...
`mulesoft-agent-fabric` (import package `agent_fabric`) rests on a small set of
architectural invariants documented in `CLAUDE.md` and the build plan
(`mulesoft-agent-fabric-sdk-build-plan.md`, the authoritative spec — every `§N.N`
...
`mulesoft-agent-fabric-sdk-build-plan.md`. When the diff adds or moves a citation:
...
product or endorses it. Keep the "for consuming MuleSoft Agent Fabric" framing.
