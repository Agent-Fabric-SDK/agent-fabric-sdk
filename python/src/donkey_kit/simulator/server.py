...
process point ``DONKEY_LLM_PROXY_URL`` at the simulator and hit it exactly
like the real proxy.
...
``donkey mock`` CLI command relies on that: it translates the
...
def serve(
    *, host: str = "127.0.0.1", port: int = 8080, config: SimulatorConfig | None = None
    ...
    print(
        ...
        "response carries 'x-donkey-simulator: true'. This is a fixture replay, "
