...
ONE object, THREE verbs, THREE trust levels (§6.3):
  ...
  * ``simulate()`` — local, developer laptop. Ephemeral local gateway; touches
    nothing shared.
  * ``export()``   — compiles to a ``donkey.yaml`` fragment (§5.1). Writes a
...
class Governance:
    ...
    def export(self, path: str | Path | None = None) -> str:
        """Compile to a ``donkey.yaml`` fragment (§5.1). Lands with M4 (§9.1)."""
    ...
    async def resolve(self, donkey: Any) -> GatewayTarget:
        ...
        :class:`~donkey_kit.core.errors.GovernanceDrift` on mismatch (§6.3).
