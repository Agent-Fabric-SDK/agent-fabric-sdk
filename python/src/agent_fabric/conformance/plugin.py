"""The public pytest plugin — ``pytest --fabric-conformance`` (#191, `BG §1.5`).
...
run in an environment where ``agent-fabric[test]`` is installed — including this
repo's own base-only job. That constraint drives the two rules this module
follows without exception:

1. **It is inert unless asked.** With no ``--fabric-conformance`` flag,
...
2. **Its module top is import-light.** It imports only stdlib, ``pytest``, and
   ...
   harness (which imports :class:`~agent_fabric.fabric.Fabric`) and ``asyncio``
...
When the flag is set, ``--fabric-conformance`` runs the conformance suite
...
def pytest_addoption(parser: pytest.Parser) -> None:
    group = parser.getgroup("agent-fabric", "Agent Fabric conformance")
    group.addoption(
        "--fabric-conformance",
        action="store_true",
        default=False,
        dest="fabric_conformance",
        help="Run the Agent Fabric conformance suite against your agent "
    ...
    group.addoption(
        ...
        "Called once per scenario; receives the fabric fixture if it declares one.",
    )
    group.addoption(
        "--fabric-known-limitations",
        action="store",
        default=None,
        dest="fabric_known_limitations",
        metavar="MODULE:NAME",
...
def _resolve_agent(config: pytest.Config) -> tuple[AgentFactory, dict[str, str]]:
    ...
    agent_spec = config.getoption("fabric_agent")
    if not agent_spec:
        raise pytest.UsageError(
            "--fabric-conformance requires --agent=module:factory "
    ...
    known_spec = config.getoption("fabric_known_limitations")
    if known_spec:
        _, known_obj = _import_target(
            known_spec, what="known limitations (--fabric-known-limitations)"
        )
    else:
...
class _ScenarioItem(pytest.Item):
    ...
    :func:`~agent_fabric.conformance.harness.run_conformance` pass (memoised on
...
class _ConformanceCollector(pytest.Collector):
    ...
    ``agent-fabric-conformance::<scenario>`` rather than dangling off the session."""
...
def pytest_collection(session: pytest.Session) -> bool | None:
    """When ``--fabric-conformance`` is set, replace normal collection with the
    ...
    if not config.getoption("fabric_conformance"):
        return None
    ...
    collector = _ConformanceCollector.from_parent(session, name="agent-fabric-conformance")
...
@pytest.fixture
def fabric() -> Any:
    """A :class:`~agent_fabric.fabric.Fabric` for tests that drive it directly
    (e.g. ``with fabric.simulate(PIIDetected): ...``).

    Built from the environment, with placeholder LLM credentials filled in only
    where absent — so ``fabric.openai()`` constructs and ``simulate()`` works
