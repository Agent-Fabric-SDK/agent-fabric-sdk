...
(:mod:`agent_fabric.conformance.plugin`) runs against a *customer's own* agent
via ``pytest --fabric-conformance --agent=my_app:build``. It is the public,
...
- **swallows PII as a generic error** — inspecting what ``run()`` lets escape:
  a bare non-:class:`~agent_fabric.core.errors.FabricError` means the typed
  refusal was lost.
...
from ..core.errors import FabricError, PIIDetected, TokenBudgetExceeded
...
PROBE_CORRELATION_ID = "agent-fabric-conformance-correlation-probe"
...
async def _swallows_pii_as_generic(ctx: ScenarioContext) -> Outcome:
    ...
    if isinstance(obs.raised, FabricError):
        return Outcome(
            False,
