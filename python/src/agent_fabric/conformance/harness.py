"""The observable ``Fabric`` the conformance suite runs against (#191, `BG §1.5`).

The suite (:mod:`agent_fabric.conformance.suite`) says *what* to check; this
module owns *how* it is observed. For each scenario it builds a fresh ``Fabric``,
...
``classify()`` is tested against (:func:`agent_fabric.simulator.inject._resolve`
...
``core``/``fabric`` surface, and the sibling simulator loader — never a web
...
# a real string.
DEFAULT_RUN_INPUT = "This is an Agent Fabric conformance probe. Reply briefly."
...
_PLACEHOLDER = "agent-fabric-conformance"
_PLACEHOLDER_URL = "https://agent-fabric-conformance.invalid"
...
def _offline_config() -> FabricConfig:
    ...
    ``fabric.openai()`` constructs without live credentials, since nothing here
...
def _fixture_responder(
    ...
    (``x-fabric-simulator: true``) as ``simulate()`` and the mock server, so a
...
class _ProbeTransport(httpx.AsyncBaseTransport, httpx.BaseTransport):
    ...
    protocols so one instance fronts a ``FabricClient`` and a
    ``FabricAsyncClient`` at once (and its count spans both)."""
...
def _build_agent(factory: AgentFactory, fabric: Fabric) -> Any:
    """Call the customer's factory, passing the ``Fabric`` iff it declares a slot
    for it — the app-factory / dependency-injection pattern (``gunicorn
    app:create_app``). ``build(fabric)`` and ``build(*, fabric=...)`` receive it;
    a zero-argument ``build()`` does not."""
    try:
        params = inspect.signature(factory).parameters
    except (TypeError, ValueError):
        # Un-introspectable (some builtins/C callables): assume it wants fabric.
        return factory(fabric)
    if "fabric" in params and params["fabric"].kind is inspect.Parameter.KEYWORD_ONLY:
        return factory(fabric=fabric)
    accepts_positional = any(
        p.kind in (inspect.Parameter.POSITIONAL_ONLY, inspect.Parameter.POSITIONAL_OR_KEYWORD)
        for p in params.values()
    ) or any(p.kind is inspect.Parameter.VAR_POSITIONAL for p in params.values())
    return factory(fabric) if accepts_positional else factory()
...
class ConformanceHarness:
    ...
    async def run_scenario(self, scenario: Scenario) -> Outcome:
        """Build a fresh ``Fabric`` + agent, run the scenario's check against
        this harness, and tear the ``Fabric`` down — so no transport swap, budget
        state, or agent state leaks between scenarios."""
        self._fabric = Fabric(_offline_config())
        self._probe = None
        try:
            self._agent = _build_agent(self._factory, self._fabric)
    ...
    def serve_refusal(self, error: type[FabricError]) -> None:
    ...
    async def run(self, *, correlation_id: str | None = None) -> Observation:
        ...
        returned: Any = None
        bind = (
            self._fabric.run_context(correlation_id)
            if correlation_id is not None and self._fabric is not None
            else nullcontext()
        )
        try:
            with bind:
    ...
    def _arm(self, responder: Callable[[httpx.Request], httpx.Response]) -> None:
        ...
        async and sync."""
        fabric = self._fabric
        assert fabric is not None  # set by run_scenario before any check runs
        self._probe = _ProbeTransport(responder)
        fabric._http._swap_transport(self._probe)
        fabric._sync_http_client()._swap_transport(self._probe)
