"""In-process refusal injection for ``fabric.simulate()`` (#190, BG §1.5).

The in-process sibling of the fabric mock server (BG §1.4): instead of a TCP
server, it swaps a fixture-returning transport onto a live ``FabricAsyncClient``
/ ``FabricClient`` for the next N calls, so a unit test can drive the refusal
...
contract; ``fabric.py`` (the orchestrator) imports it lazily inside
:meth:`Fabric.simulate`.
"""
...
_EXC_TO_SHAPE: dict[type[FabricError], str] = {
...
class _SwappableClient(Protocol):
    """The transport-swap seam both Fabric HTTP clients expose (§2.3, BG §1.1).
...
class _Countdown:
    ...
    sync surfaces together — 'the next N calls through a Fabric'."""
...
class _FixtureTransport(httpx.AsyncBaseTransport, httpx.BaseTransport):
    ...
    *logical* calls, then delegating so call N+1 proceeds normally.
    ...
    front either a ``FabricClient`` or a ``FabricAsyncClient``.
...
def _resolve(error: type[FabricError]) -> Fixture:
    ...
    if not (isinstance(error, type) and issubclass(error, FabricError)):
        raise TypeError(
            f"simulate() expects a FabricError subclass, got {error!r}. "
