...
:mod:`agent_fabric.simulator.fixtures`) so a stock ``openai``/``httpx`` client —
or ``fabric.llm.client()`` — sees byte-identical rejection bodies and the exact
...
carries ``x-fabric-simulator: true`` so it can never be mistaken for a real
gateway (BG §1.4, non-negotiable).

Framework isolation (§1.1): ``starlette`` is imported **lazily inside**
:func:`build_app`, never at module top, so ``import agent_fabric.simulator``
...
SIMULATOR_HEADER = "x-fabric-simulator"
...
# whose model id is "fabric-sim/<shape>" is served that rejection shape. This is
# a simulator control surface only — NEVER a real Omni Gateway behaviour (§0.3).
SIM_MODEL_PREFIX = "fabric-sim/"
...
class SimulatorConfig:
    ...
    (#253) — it exists so a developer can exercise :class:`~agent_fabric.Budget`
...
class _HonestyStamp:
    ...
    :meth:`_Simulator._response`. It adds ``x-fabric-simulator: true`` to any
...
def build_app(config: SimulatorConfig | None = None) -> ASGIApp:
    ...
    :func:`agent_fabric.simulator.server.serve` for a real TCP port.

    The app is wrapped in :class:`_HonestyStamp` so the ``x-fabric-simulator``
