...
1. Packaged ``agent_fabric/simulator/_fixtures/…`` — present in a built wheel via
   the ``[tool.hatch.build.targets.wheel.force-include]`` mapping in
   ``pyproject.toml`` (so a ``pip install``-ed ``agent-fabric mock`` finds them).
...
# python/src/agent_fabric/simulator/fixtures.py, so parents[3] is python/.
...
def _packaged_bytes(directory: str, name: str) -> bytes | None:
    ...
    res = importlib.resources.files("agent_fabric.simulator")
...
def fixture_bytes(directory: str, name: str) -> bytes:
    ...
    raise FileNotFoundError(
        ...
        f"agent_fabric/simulator/_fixtures/ nor present at {_SOURCE_ROOT}. "
...
def load(shape: str) -> Fixture:
    ...
    which headers to replay vs. strip; see :mod:`agent_fabric.simulator.app`.
    """
    try:
        spec = SHAPES[shape]
    except KeyError:
