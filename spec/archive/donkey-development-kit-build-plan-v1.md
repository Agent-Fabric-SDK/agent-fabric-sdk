# Agent Fabric SDK — Build Plan & Implementation Spec (v1, ARCHIVED)

> ## ⚠️ ARCHIVED — not the authority for what to build
>
> This is the **v1** build plan, superseded by
> [`../agent-fabric-sdk-build-plan.md`](../agent-fabric-sdk-build-plan.md) and
> [`../agent-fabric-sdk-build-guide.md`](../agent-fabric-sdk-build-guide.md).
> Its three-pillar model, provisioning control plane, and eight-adapter
> conformance roster were all **cut**. Do not plan, scope, or review work
> against this document.
>
> **It is retained for one reason:** roughly 500 `§N.N` citations in
> `python/` source, tests, docstrings, and `docs/` point into this
> document's numbering. Until those are migrated, a bare `§N.N` citation
> anywhere in this repo resolves **here**, not into the new plan. See
> "Citation convention" in the new build plan.
>
> Sections that describe **still-live, CI-enforced invariants** — `§0.3`
> verification discipline, `§1.1` layered architecture and framework-free
> core, `§2.1` config resolution, `§8.1` conformance kit, `§8.4` extras are
> floors-never-ceilings — remain accurate and are carried forward by
> reference. Everything about the three pillars (`§3`, `§4`, `§5`),
> publication (`§7`), and the provisioning control plane is historical.

**Audience:** an engineering agent (Claude Opus 4.8 / Sonnet 5) implementing the SDK, plus the human tech lead reviewing scope.
**Status:** ARCHIVED design spec — superseded, retained for citation resolution.
