...
- **Connection lifecycle.** MCP clients are stateful, and several frameworks
  connect lazily. `fabric.tools.discover()` never opens a connection — it
...
```python
tools = await fabric.tools.discover(domain="hr")
...
| MS Agent Framework | its MCP client/tool class for streamable HTTP (name pending live verification — §9); the framework's own docs reference hosted MCP tools and MCP clients for tool integration. |
| OpenAI Agents SDK | `agents.mcp.MCPServerStreamableHttp(params={"url": ..., "headers": ...})`, passed into `Agent(mcp_servers=[...])` (pending live verification — §9). |
| Anthropic SDK | no native client-side MCP binding in the `anthropic` SDK itself; bind via the MCP Python SDK's streamable-HTTP client and pass the resulting tool schemas to `messages.create(tools=...)` (pending live verification — §9). |
| CrewAI | its MCP adapter for streamable-HTTP servers (name pending live verification — §9), yielding native `crewai` tool objects for a `Crew`/`Agent`. |
