...
MuleSoft LLM proxy through ADK's `LiteLlm` model wrapper, which speaks
...
async with Fabric.from_env() as fabric:
    llm = LiteLlm(model="openai/gpt-4o", **fabric.adk.connection_kwargs())
```
