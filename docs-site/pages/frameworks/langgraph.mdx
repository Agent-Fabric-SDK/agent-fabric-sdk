...
the MuleSoft LLM proxy, with full header and transport injection.

> **Tier 1 — full conformance-gated.** This is the best-case adapter: every
...
const client = new OpenAI({
  baseURL: process.env.MULESOFT_LLM_PROXY_URL,   // no /v1
...
llm = ChatOpenAI(
    model="gpt-4o",
    base_url=...,             # from MULESOFT_LLM_PROXY_URL, no /v1 suffix
    api_key=...,
...
> layer (§2.3), not in the OpenAI client, so the two don't double-retry.
