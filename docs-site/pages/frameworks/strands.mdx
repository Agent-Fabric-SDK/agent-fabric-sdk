...
> **Tier 1 — full conformance-gated.** `client_args` gives full header AND
...
async with Fabric.from_env() as fabric:
    llm = fabric.strands.model("gpt-4o")
```
...
async with Fabric.from_env() as fabric:
    llm = OpenAIModel(model_id="gpt-4o", **fabric.strands.connection_kwargs())
```
