...
MuleSoft LLM proxy, with the SDK's shared transport and verified headers
injected directly into the client constructor.

> **Tier 1 — full conformance-gated**, with one important divergence from
...
const client = new Anthropic({
  baseURL: process.env.MULESOFT_LLM_PROXY_URL,   // UNVERIFIED: needs an Anthropic-native route
...
async with Fabric.from_env() as fabric:
    llm = AsyncAnthropic(**fabric.anthropic.connection_kwargs())
```
...
> to `.messages.create()`, not to the constructor. `fabric.anthropic.client()`
> therefore takes no model argument at all; you supply the model id yourself
> on every call.

> **UNVERIFIED: the proxy's Anthropic-native Messages route (open M0 item).**
