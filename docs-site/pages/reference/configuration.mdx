...
| Env var | Meaning |
|---|---|
| `AGENT_FABRIC_LLM_PROXY_URL` | Proxy base URL: `https://<ingress-gw>/<instance>/` — **no `/v1`**. |
| `AGENT_FABRIC_LLM_PROXY_CLIENT_ID` | Consumer client id (the per-agent identity). |
| `AGENT_FABRIC_LLM_PROXY_CLIENT_SECRET` | Consumer client secret. |
...
| Env var | Meaning |
|---|---|
| `AGENT_FABRIC_APP_NAME` | Human-readable app name, surfaced on telemetry. |
| `AGENT_FABRIC_BUSINESS_GROUP` | Business group for attribution. |
