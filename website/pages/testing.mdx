...
<Callout type="info">
  **Phase 1 — `simulate()` ships; the conformance plugin is the follow-up.**
  In-process refusal injection (`fabric.simulate()`) is shipped. The pytest
  conformance plugin below is designed, not yet shipped — its API and CLI
  shapes are a proposal, tracked as
  [#191](https://github.com/Agent-Fabric-SDK/agent-fabric-sdk/issues/191). See
...
A context manager that makes the next N calls through a `Fabric` return a
...
```python
from agent_fabric import Fabric, PIIDetected
...
Every injected response carries `x-fabric-simulator: true`, so a simulated
...
[#253](https://github.com/Agent-Fabric-SDK/agent-fabric-sdk/issues/253)).
...
<Callout type="info">
  ...
  follow-up [#188](https://github.com/Agent-Fabric-SDK/agent-fabric-sdk/issues/188),
  shared with the simulator's scenario scripting.
</Callout>

## The conformance plugin — run it against *your* agent

This is the part that matters. `pip install "agent-fabric[test]"` publishes a
pytest plugin exposing a `fabric` fixture and a conformance suite you point at
your own agent:
...
| Works with a stock OpenAI client | Yes | No — it hooks the `Fabric` transport |
...
- The plugin registers as a pytest entry point, and
  `pytest --fabric-conformance` prints a scenario table —
  [#191](https://github.com/Agent-Fabric-SDK/agent-fabric-sdk/issues/191).
- Exemptions must be asserted in code; a silent skip fails the suite —
  [#191](https://github.com/Agent-Fabric-SDK/agent-fabric-sdk/issues/191).

---

**Status: Phase 1 — `simulate()` is shipped; the conformance plugin is
[#191](https://github.com/Agent-Fabric-SDK/agent-fabric-sdk/issues/191).**
