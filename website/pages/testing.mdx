...
<Callout type="info">
  **Phase 1 — both ship.** In-process refusal injection (`fabric.simulate()`)
  and the pytest conformance plugin
  ([#191](https://github.com/Agent-Fabric-SDK/agent-fabric-sdk/issues/191)) are
  both shipped. The plugin runs your agent **in process** through its own
  `Fabric` — there is no separate server mode. See [Roadmap](/roadmap) and
...
- The plugin registers as a pytest entry point, and
  `pytest --fabric-conformance --agent=my_app.agent:build` turns each scenario
  into one pytest item and prints a scenario table. ✅
- Exemptions must be asserted in code — a `KNOWN_LIMITATIONS` mapping next to
  your factory (or `--fabric-known-limitations=module:NAME`), validated at
