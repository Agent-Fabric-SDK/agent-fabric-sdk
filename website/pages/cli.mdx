...
<Callout type="info">
  **Phase 1 — mostly designed, not yet shipped.** One command has landed: the
  local simulator, run as `agent-fabric mock` (see [Local simulator](/simulator)).
...
`@fabric.tool` marks a function as a governed tool. It does nothing on its own
...
```bash
fabric init          # writes .agent-fabric.toml, prints which env vars are missing
fabric doctor        # checks creds, reaches the gateway, reports budget state
fabric mock          # the local simulator — SHIPPED, run as: agent-fabric mock
fabric test          # the conformance suite
```

### `fabric doctor` is the one that pays for itself
...
- `fabric doctor` distinguishes those three failures from each other — not one
...
**Status: Phase 1 — `agent-fabric mock` (the local simulator) is shipped;
