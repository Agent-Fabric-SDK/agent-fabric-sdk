...
async with Fabric.from_env() as fabric:
    llm = fabric.openai_agents.model("gpt-4o")
```
...
async with Fabric.from_env() as fabric:
    llm = OpenAIChatCompletionsModel(
        model="gpt-4o",
        **fabric.openai_agents.connection_kwargs(),
    )
```
...
> OpenAI Python SDK. Installing `agent-fabric[openai-agents]` pulls in
> `openai-agents` for you. For the raw governed client with no framework, use
> `fabric.openai()` (from `agent-fabric[llm]`) instead.
