...
`fabric.tools.discover(...)` is the one entry point for finding governed tools.
...
tools = await fabric.tools.discover(domain="hr", governed_only=True)
```
...
```python
from agent_fabric.registry.governance import GovernanceCriteria, STRICT
...
```python
tools = await fabric.tools.discover(domain="hr", governed_only=STRICT)
```
...
- **Warm the index.** The governance join is a bulk operation, not one API call
  ...
  `fabric.registry.warm(environment=...)` so first discovery is fast.
