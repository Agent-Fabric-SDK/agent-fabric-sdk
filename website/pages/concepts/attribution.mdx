...
```python
async with fabric.run(id="order-1234"):
    ...  # calls in here share the run id
```

The run id becomes the `X-Correlation-Id` request header, the span's
`fabric.correlation_id`, and `FabricError.correlation_id`; each individual
request additionally carries a per-call `X-Fabric-Request-Id` that surfaces on
`FabricError.call_id`. See [Observability](/concepts/observability#grouping-a-run-the-run-id-on-the-span-the-call-id-on-the-error)
