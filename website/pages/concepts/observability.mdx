...
| `fabric.policy.decision` | `fabric.*` (stable) | `allow` on success; `refuse` on a classified policy refusal (which also sets the span's OTel status to `ERROR`). |
...
| `fabric.correlation_id` | `fabric.*` (stable) | The **run** correlation id actually sent on the request (the `X-Correlation-Id` header) — shared by every call in a `fabric.run()` block. The per-call id is deliberately *not* on the span; it lives on `FabricError.call_id` (see below). |
...
`with fabric.run(...)` works too. Inside the block:

- The **run id** becomes the `X-Correlation-Id` request header, the span's
  `fabric.correlation_id`, and `FabricError.correlation_id`. It is shared by
...
- Each individual request also gets a fresh **per-call id** (the
  `X-Fabric-Request-Id` header), unique per logical request and stable across
  that request's retries. It surfaces on `FabricError.call_id` — **not** on the
...
<Callout type="info">
  ...
  Whether it *reads* an inbound `X-Correlation-Id` (and `X-Fabric-Request-Id`) as
...
- **A refusal is a failed operation.** When the proxy rejects a call and
  `classify()` maps it to a policy refusal, the span carries `fabric.policy.decision
  = refuse` and the `fabric.policy.type` slug *and* its OTel status is set to
