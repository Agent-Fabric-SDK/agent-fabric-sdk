...
<Callout type="info">
  **Phase 1 — the `Budget` object ships; pacing is next.** The object below
  (`limit` / `remaining` / `reset_at` / `observed_at` / `fraction_used`), updated
  in-band from every response, is implemented. The two pacing helpers —
  `pace()` and `wait_for_reset()` — are designed but **not yet shipped**; that
  API shape is still a proposal. See [Roadmap](/roadmap) and
...
`Budget` object on the `Fabric` instance.

## The object

```python
fabric.budget.limit          # int, tokens per window
fabric.budget.remaining      # int, from the last response
fabric.budget.reset_at       # datetime, converted from x-token-reset (ms) — not raw
fabric.budget.observed_at    # when headers were last seen (staleness)
fabric.budget.fraction_used  # 0.0–1.0
```

And two helpers that use it — **designed, not yet shipped** (the next piece):
...
- `pace()` raises before the request that would cross the reserve, never after
  a `429`.
- The budget object is per-`Fabric`, not global: two instances with different
  credentials do not share state.

---

**Status: Phase 1 — the `Budget` object is shipped; `pace()` / `wait_for_reset()` are next.**
