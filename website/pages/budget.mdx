...
<Callout type="info">
  **Phase 1 — the `Budget` object and its pacing helpers ship.** The object
  ...
  test against the local gateway simulator* (`fabric mock`), which lands with the
...
**Status: Phase 1 — the `Budget` object, `pace()`, and `wait_for_reset()` are shipped. The end-to-end simulator test is deferred to the local gateway simulator (`fabric mock`).**
