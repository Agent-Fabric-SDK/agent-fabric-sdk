...
`fabric scan` walks your repository, finds everything marked
[`@fabric.tool`](/cli), plus MCP server definitions and agent entry points, and
produces a manifest and A2A agent card. `fabric publish` registers it with
...
`fabric publish --cross-check` runs `auto` and `auto:live` and diffs them.
