...
| Prompt-injection / content-safety bodies | Not captured | Injection is typed by the `x-injection-protection: blocked` header (→ `PromptInjectionBlocked`), but its **body** is uncaptured; content-safety still falls through to a generic `PolicyViolation`. No invented discriminator — bodies tracked in #253. |
