...
`UpstreamRequestError`, `UpstreamModelError`, `AuthError`, and the generic
`PolicyViolation` (the still-under-documented content-moderation shape,
[#253](https://github.com/Donkey-Development-Kit/donkey-development-kit/issues/253)).
Asking for a refusal with no captured fixture — e.g. `ContentSafetyBlocked` —
is a clear `ValueError`, never a silent miss.
